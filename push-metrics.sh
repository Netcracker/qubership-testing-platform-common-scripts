...
#   ENVIRONMENT_NAME, CURRENT_DATE, CURRENT_TIME
...
# Args: endpoint_type base_url env run_date run_time
_metrics_build_url() {
    local endpoint_type="$1"
    local base_url
    base_url="$(_metrics_base_trim "$2")"
    local env="$3" run_date="$4" run_time="$5"

    case "$endpoint_type" in
        pushgateway)
            printf '%s' "${base_url}/metrics/job/atp_playwright_runner/instance/${env}/${run_date}/${run_time}"
            ;;
        vm-native)
            local j i rd rt
            j=$(printf '%s' 'job=atp_playwright_runner' | jq -sRr @uri)
            i=$(printf 'instance=%s' "$env" | jq -sRr @uri)
            rd=$(printf 'run_date=%s' "$run_date" | jq -sRr @uri)
            rt=$(printf 'run_time=%s' "$run_time" | jq -sRr @uri)
            printf '%s' "${base_url}/api/v1/import/prometheus?extra_label=${j}&extra_label=${i}&extra_label=${rd}&extra_label=${rt}"
            ;;
        *)
...
# Args: caller_fn_name payload env run_date run_time
_metrics_dispatch() {
    local caller="$1" payload="$2" env="$3" run_date="$4" run_time="$5"
...
push_metrics() {
    ...
    local run_date="${CURRENT_DATE:-$(date +%F)}"
    local run_time="${CURRENT_TIME:-$(date +%H-%M-%S)}"
    ...
    payload+="atp_test_suite_pass_rate{environment=\"${env}\",name=\"${safe_suite_name}\",overall_status=\"${overall_status}\"} ${pass_rate}\n"

    # --- Run-level counts ---
    payload+="# HELP atp_test_suite_total Total number of test cases in the run\n"
    payload+="# TYPE atp_test_suite_total gauge\n"
    payload+="atp_test_suite_total{environment=\"${env}\",name=\"${safe_suite_name}\",overall_status=\"${overall_status}\"} ${total}\n"

    payload+="# HELP atp_test_suite_passed Number of passed test cases in the run\n"
    payload+="# TYPE atp_test_suite_passed gauge\n"
    payload+="atp_test_suite_passed{environment=\"${env}\",name=\"${safe_suite_name}\",overall_status=\"${overall_status}\"} ${passed}\n"

    payload+="# HELP atp_test_suite_failed Number of failed test cases in the run\n"
    payload+="# TYPE atp_test_suite_failed gauge\n"
    payload+="atp_test_suite_failed{environment=\"${env}\",name=\"${safe_suite_name}\",overall_status=\"${overall_status}\"} ${failed}\n"

    payload+="# HELP atp_test_suite_skipped Number of skipped test cases in the run\n"
    payload+="# TYPE atp_test_suite_skipped gauge\n"
    payload+="atp_test_suite_skipped{environment=\"${env}\",name=\"${safe_suite_name}\",overall_status=\"${overall_status}\"} ${skipped}\n"
    ...
    if [[ "$allure_has_results" == "true" ]]; then
        for result_file in "$allure_results_dir"/*-result.json; do
            ...
            payload+="atp_test_case_result{test_name=\"${safe_name}\",environment=\"${env}\",suite=\"${safe_suite}\",status=\"${status}\"} ${result_val}\n"
        ...
        for ((i=0; i<count; i++)); do
            ...
            safe_name="${safe_name//$'\n'/}"
            safe_name="${safe_name//$'\r'/}"
            payload+="atp_test_case_result{test_name=\"${safe_name}\",environment=\"${env}\",suite=\"\",status=\"${status}\"} ${result_val}\n"
        done
    fi
